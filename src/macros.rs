macro_rules! blake2_impl {
    (
        $name:ident, $fn_name:ident, $alg_name:expr, $word:ident, $vec:ident, $bytes:ident,
        ...
        pub fn $fn_name(
            ...
            let mut v = [
                ...
                h[1],
                $name::iv0(),
                $name::iv1() ^ $vec::new(t0, t1, f0, f1),
            ];

            for x in 1..rounds + 1 {
                let x = if x > 10 { x - 11 } else { x - 1 };
                $name::round(&mut v, &m, &SIGMA[x as usize]);
            }
        ...
        impl $name {
            ...
            fn round(v: &mut [$vec; 4], m: &[$word; 16], s: &[usize; 16]) {
                Self::quarter_round(v, $R1, $R2, $vec::gather(m, s[0], s[2], s[4], s[6]));
                Self::quarter_round(v, $R3, $R4, $vec::gather(m, s[1], s[3], s[5], s[7]));

                Self::shuffle(v);
                Self::quarter_round(v, $R1, $R2, $vec::gather(m, s[8], s[10], s[12], s[14]));
                Self::quarter_round(v, $R3, $R4, $vec::gather(m, s[9], s[11], s[13], s[15]));
                Self::unshuffle(v);
            }
